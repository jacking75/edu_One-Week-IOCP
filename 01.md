...
네 번째로, IOCP는 워커 스레드 실행 수를 커널 수준에서 조절한다. 완료 패킷은 Completion Port 큐에 FIFO 방식으로 들어가지만, 대기 중인 워커 스레드는 LIFO 방식, 즉 가장 최근에 대기한 스레드가 먼저 깨어난다. 이 방식은 같은 스레드가 연속해서 작업을 처리할 가능성을 높여 컨텍스트 스위칭을 줄이고 캐시 효율을 높이는 데 도움이 된다.    
