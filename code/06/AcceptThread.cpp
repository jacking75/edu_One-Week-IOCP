...
static std::atomic<uint64_t> g_sessionIdGenerator(1);
...
void ProcessAccept(AcceptOverlapped* acceptOv, DWORD bytesTransferred) {
    ...
    uint64_t sessionId = g_sessionIdGenerator.fetch_add(1);
    auto session = std::make_shared<Session>(acceptOv->acceptSocket, sessionId);
