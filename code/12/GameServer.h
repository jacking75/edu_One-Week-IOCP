...
public:
    GameServer()
        : hCompletionPort_(NULL), listenSocket_(INVALID_SOCKET)
        , isRunning_(false), nextSessionId_(1) {}
    ~GameServer() { if (isRunning_) Shutdown(); }
...
private:
    ...
    void HandleOmokPacket(SessionPtr pSession, PacketId packetId,
